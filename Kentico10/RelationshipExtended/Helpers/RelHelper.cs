...
namespace RelationshipsExtended
{
    public static class RelHelper
    {
        ...
        public static string GetDocumentCategoryWhere(IEnumerable<object> Values, ConditionType Condition = ConditionType.Any, string DocumentIDTableName = "CMS_Document")
        {
            ...
            }, new CacheSettings((CacheWhere ? CacheHelper.CacheMinutes(SiteContext.CurrentSiteName) : 0), "GetDocumentCategoryWhere", Values, Condition, DocumentIDTableName));
        }
        ...
        public static string GetNodeCategoryWhere(IEnumerable<object> Values, ConditionType Condition = ConditionType.Any, string NodeIDTableName = "CMS_Tree")
        {
            ...
            }, new CacheSettings((CacheWhere ? CacheHelper.CacheMinutes(SiteContext.CurrentSiteName) : 0), "GetNodeCategoryWhere", Values, Condition, NodeIDTableName));
        }

        /// <summary>
        /// Returns a full where condition (for Binding tables that bind an object to Categories) to be used in filtering (ex repeaters).  
        /// </summary>
        /// <param name="BindingClass">The Binding Class Code Name</param>
        /// <param name="ObjectIDFieldName">The Field Name of this object that matches the binding table's Left Field value</param>
        /// <param name="LeftFieldName">The Field Name of the binding class that contains this Object IDs value</param>
        /// <param name="RightFieldName">The Field Name of the binding class that contains the Category's identy value</param>
        /// <param name="Values">list of category values (int IDs, GUIDs, or string CodeNames)</param>
        ...
        /// <param name="Condition">RelEnums.ConditionType of what type of condition to generate.</param>
        /// <param name="ObjectIDTableName">The Table Name/Alias where the ObjectIDFieldName belongs. Only needed for the 'All' Condition and if the ObjectIDField and LeftFieldName are the same.</param>
        /// <returns>The Where Condition, If no categories provided or none found, returns 1=1</returns>
        public static string GetBindingCategoryWhere(string BindingClass, string ObjectIDFieldName, string LeftFieldName, string RightFieldName, IEnumerable<object> Values, IdentityType Identity = IdentityType.ID, ConditionType Condition = ConditionType.Any, string ObjectIDTableName = null)
        {
            ...
            }, new CacheSettings((CacheWhere ? CacheHelper.CacheMinutes(SiteContext.CurrentSiteName) : 0), "GetBindingCategoryWhere", BindingClass, ObjectIDFieldName, LeftFieldName, RightFieldName, Values, Identity, Condition, ObjectIDTableName));
        }

        /// <summary>
        /// Returns a full where condition (for Binding Tables that bind on any object) to be used in filtering (ex repeaters).
        /// </summary>
        /// <param name="BindingClass">The Binding Class Code Name</param>
        /// <param name="ObjectClass">The Object Class Code Name (the thing that is bound to the current object through the binding table)</param>
        /// <param name="ObjectIDFieldName">The Field Name of this object that matches the binding table's Left Field value</param>
        /// <param name="LeftFieldName">The Field Name of the binding class that contains this Object IDs value</param>
        /// <param name="RightFieldName">The Field Name of the binding class that contains the related objects's identy value</param>
        ...
        public static string GetBindingWhere(string BindingClass, string ObjectClass, string ObjectIDFieldName, string LeftFieldName, string RightFieldName, IEnumerable<string> Values, IdentityType Identity = IdentityType.ID, ConditionType Condition = ConditionType.Any, string ObjectIDTableName = null)
        {
            ...
            }, new CacheSettings((CacheWhere ? CacheHelper.CacheMinutes(SiteContext.CurrentSiteName) : 0), "GetBindingWhere", BindingClass, ObjectClass, ObjectIDFieldName, LeftFieldName, RightFieldName, Values, Identity, Condition, ObjectIDTableName));
        }
        ...
        public static void HandleNodeBindingInsertUpdateDeleteEvent(int NodeID, string BindingObjectClassName, int TaskOriginatorUserID = -1, int TimerMS = 2000)
        {
            ...
            if (CreateTimer)
            {
                ...
                TheTimer.Elapsed += (object TimerObj, ElapsedEventArgs ElapsedEvent) =>
                {
                    ...
                    // Reset the Membership Context so the task is logged by the user who initiated it.
                    ...
                    }, new CacheSettings(60, "HandleNodeBindingInsertUpdateDeleteEvent_User", TaskOriginatorUserID));
            ...
            foreach (DataTable BoundObjectTable in e.TaskData.Tables.Cast<DataTable>().Where(x => x.TableName.ToLower() == BoundObjectClassNameTableName))
            {
                foreach (DataRow BoundObjectDR in BoundObjectTable.Rows)
                {
                    ...
                    if (TaskBoundIDs.Contains(ObjectID))
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(BoundObjectTypeInfo.GUIDColumn.Replace(ObjectTypeInfo.COLUMN_NAME_UNKNOWN, "")))
                        {
                            ObjectParams.Guid = ValidationHelper.GetGuid(BoundObjectDR[BoundObjectTypeInfo.GUIDColumn], Guid.Empty);
                        }
                        if (!string.IsNullOrWhiteSpace(BoundObjectTypeInfo.CodeColumn.Replace(ObjectTypeInfo.COLUMN_NAME_UNKNOWN, "")))
                        {
                            ObjectParams.CodeName = ValidationHelper.GetString(BoundObjectDR[BoundObjectTypeInfo.CodeColumn], "");
                        }
                        if (!string.IsNullOrWhiteSpace(BoundObjectTypeInfo.SiteIDColumn.Replace(ObjectTypeInfo.COLUMN_NAME_UNKNOWN, "")))
                        {
